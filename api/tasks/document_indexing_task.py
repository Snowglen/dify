...
def _document_indexing_with_tenant_queue(
    ...
    try:
        ...
        if next_tasks:
            # Dispatch follow-up tasks using the standard Celery .delay API so tests
            # can easily mock and assert calls, while still preserving tenant
            # isolation semantics.
            with current_app.producer_or_acquire() as producer:  # type: ignore
                for next_task in next_tasks:
                    ...
                    task_func.apply_async(  # type: ignore[attr-defined]
                        kwargs={
