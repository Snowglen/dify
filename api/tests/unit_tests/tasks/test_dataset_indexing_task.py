    ...
    with patch("tasks.document_indexing_task.session_factory") as mock_sf:
        ...
        def _query_side_effect(model):
            ...
            if model.__name__ == "Dataset":
                q.first.side_effect = lambda: session._shared_data.get("dataset")
                q.all.side_effect = lambda: (
                    [session._shared_data.get("dataset")] if session._shared_data.get("dataset") else []
                )
                return q

            # Document queries
            if model.__name__ == "Document":
                q.all.side_effect = lambda: list(session._shared_data.get("documents", []))

                def _first_doc():
                    docs = session._shared_data.get("documents", [])
                    if not docs:
                        return None
                    # Return next doc each call to mimic filtering by id in tests that loop
                    idx = session._doc_first_idx % len(docs)
                    session._doc_first_idx += 1
                    return docs[idx]

                q.first.side_effect = _first_doc
                return q
